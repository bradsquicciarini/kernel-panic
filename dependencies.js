...
{ 
  constructor()  
    ...
    // VERY SIMPLE.
    // (1) Need to check for collisions for the smaller two platforms 
    collisions_helper(platform, player)
    {
      // Calculate distance between player and top edge of stage
      var distY = Math.abs(player.position.y - platform.y);
      var distX = Math.abs(player.position.x - platform.x);
